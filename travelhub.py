...
if st.session_state.page not in ["login", "signup"]:
    ...
    with st.sidebar.form("nav_form",clear_on_submit=False):
        ...
        flight_btn = st.form_submit_button("✈️ Book-Flight Ticket",use_container_width=True)
...
def home():
    ...
    def showResults():
        ...
        with st.container():
            ...
            with col2:
                st.markdown(f"⏱️ **{search_results[6]}**")
            with col3:
    ...
    def showClass():
        ...
        for i in range(passengernum):
            ...
            if name and age and gender:
                ...
                st.error("⚠️ Please fill all fields properly before booking.")
        ...
        if st.session_state.tickets:
            for t in st.session_state.tickets:
                ...
                <div class='ticket-card'>
                    <b>🎫 Ticket ID:</b> {t['ticket_id']}<br>
                    <b>👤 Name:</b> {t['name']}<br>
                    <b>🚇 Metro:</b> {t['train']}<br>
                    <b>📅 Date:</b> {t['date']}
                </div>
                """, unsafe_allow_html=True)
        else:
